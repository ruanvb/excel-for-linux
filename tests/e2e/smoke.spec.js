...
test('app launches and redirects to Microsoft login', async () => {
  ...
  try {
    ...
    userDataDir = mkdtempSync(join(tmpdir(), 'excel-e2e-'));
    ...
    // Get all windows and find the main Excel window
    const windows = electronApp.windows();
    expect(windows.length).toBeGreaterThan(0);

    // Find the main window (not a toast/auxiliary window)
    const mainWindow = windows.find(w => {
      const url = w.url();
      try {
        const hostname = new URL(url).hostname;
        return hostname === 'www.office.com' ||
               hostname === 'office.com' ||
               hostname === 'excel.cloud.microsoft' ||
    ...
    // Wait for the login page DOM to be ready

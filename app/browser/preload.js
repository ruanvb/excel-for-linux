...
function createWebNotification(classicNotification, title, options) {
  ...
  // Return actual native notification object (critical for the app to manage lifecycle)
...
// Override window.Notification immediately before the app loads
...
(function() {
  ...
  function CustomNotification(title, options) {
    ...
    if (notificationConfig?.disableNotifications) {
      // Return dummy object to avoid app errors
      return { onclick: null, onclose: null, onerror: null };
    }
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    const modulesRequiringIpc = new Set(["settings", "theme", "trayIconRenderer"]);

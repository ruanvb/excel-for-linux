...
class CommandLineManager {
  ...
  static addSwitchesAfterConfigLoad(config) {
    ...
    // Always set app name so GNOME/KDE dock can match the window to the .desktop entry.
    // Falls back to 'excel-for-linux' (matches StartupWMClass in the .desktop file).
    const appClass = config.class || 'excel-for-linux';
    if (config.class) {
      console.info("Setting WM_CLASS property to custom value " + config.class);
    }
    app.setName(appClass);

 ...
 * from excel-for-linux.appdata.xml file, ensuring version consistency across package files.
 *
...
async function generateReleaseInfo(projectRoot = null) {
  ...
  // Load excel-for-linux.appdata.xml
  const appdataPath = path.join(
    root,
    "excel-for-linux.appdata.xml"
  );
  if (!fs.existsSync(appdataPath)) {
    throw new Error(
      "excel-for-linux.appdata.xml not found."
    );
  }
  ...
  if (!component?.releases?.[0]?.release) {
    throw new Error(
      "No releases found in excel-for-linux.appdata.xml."
    );
  }
  ...
  if (!matchingRelease) {
    throw new Error(
      `No release entry found for version ${baseVersion} in excel-for-linux.appdata.xml. Please add a release entry for this version.`
    );
  }
  ...
  if (!releaseNotes.trim()) {
    throw new Error(
      `Release ${baseVersion} has no description/notes in excel-for-linux.appdata.xml. Please add release notes to the <description> section for this version.`
    );
  }
...
if (require.main === module) {
  (async () => {
    try {
      ...
      console.log(
        `   excel-for-linux.appdata.xml: ${versionInfo.appdata} (with release notes)`
      );
      console.log("");
      console.log(

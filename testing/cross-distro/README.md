...
Docker-based manual testing for Excel for Linux across distributions and display servers.
...
./run.sh ubuntu x11 --url https://github.com/your-username/excel-for-linux/releases/download/v0.1.0/excel-for-linux-0.1.0.AppImage

# From local AppImage
./run.sh ubuntu x11 --appimage ../../dist/excel-for-linux-*.AppImage
...
./run.sh --run-distro fedora --url https://example.com/excel.AppImage
...
./run.sh --run-all
./run.sh --run-all --url https://example.com/excel.AppImage --no-launch
```
...
```bash
APP_URL=https://example.com/excel.AppImage docker compose up --build ubuntu-x11 fedora-wayland
...
```bash
# X11 / XWayland
/home/tester/app-local/excel-for-linux.AppImage --appimage-extract-and-run \
...
curl -fSL -o app/excel-for-linux.deb \
  https://github.com/your-username/excel-for-linux/releases/download/v0.1.0/excel-for-linux_0.1.0_amd64.deb
...
```bash
curl -fSL -o app/excel-for-linux.rpm \
  https://github.com/your-username/excel-for-linux/releases/download/v0.1.0/excel-for-linux-0.1.0.x86_64.rpm
...
**AppImage fails:** Extract manually: `./excel-for-linux.AppImage --appimage-extract && ./squashfs-root/excel-for-linux --no-sandbox`
